                ...
                <div class="card-header bg-white py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('teacher.students.show', $student->id) }}" class="btn btn-info btn-sm">
                    ...
                    <p class="text-muted mb-0 mt-2">Edit student information for {{ $student->full_name }}.</p>
                </div>

                <div class="card-body p-4">
                    <form action="{{ route('teacher.students.update', $student->id) }}" method="POST" id="studentEditForm">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control @error('birth_date') is-invalid @enderror"
                                            id="birth_date" name="birth_date" value="{{ old('birth_date', $student->birth_date ? $student->birth_date->format('Y-m-d') : '') }}" required>
                                        @error('birth_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                        ...
                                        <select class="form-select @error('section_id') is-invalid @enderror"
                                            ...
                                            @foreach($sections as $section)
                                                <option value="{{ $section->id }}" {{ old('section_id', $student->section_id) == $section->id ? 'selected' : '' }}>
                                                    {{ $section->name }} ({{ $section->grade_level ?? 'Unknown Grade' }}) - {{ $section->school_year }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('section_id')
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const submitBtn = document.querySelector('#studentEditForm button[type="submit"]');

        if (studentForm && submitBtn) {
            console.log('Found student edit form and submit button');
            ...
            console.error('Could not find student edit form or submit button');
        }
    });
</script>
@endpush
